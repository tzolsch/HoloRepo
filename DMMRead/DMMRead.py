...
class Dmm:
    ...
    def __init__(self, port='COM3', retries=3, timeout=3.0):
...
if __name__ == "__main__":
    dmm = Dmm(port='COM4')
    v = dmm.read().numericVal
    ind = pd.Index(['DMM'], name='names')
    s = pd.DataFrame(0, index=ind, columns=['values'])
    while True:
        v = dmm.read().numericVal
        s[:] = v
        s.to_csv('D:\TDdata\Readings\DMM.txt',';')